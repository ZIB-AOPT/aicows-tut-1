...
The `kit/` folder is the client-side half of the arena project. The
...
| Path | Contents |
|---|---|
| `kit/hiddengems/sdk.py` | Python SDK: `BotStrategy`, `MappingBot`, `run_bot` |
| `kit/hiddengems/config.py` | public constants and defaults |
| `kit/examples/python/bot.py` | minimal Python strategy |
| `kit/rust/` | complete Rust client; strategy in `src/bot.rs` |
| `kit/cpp/` | complete C++ client; strategy in `src/bot.hpp` |
| `kit/current-rules.md` | authoritative mechanics |
| `kit/protocol.md` | wire format, deadlines, failure handling |
...
| | Python | Rust | C++ |
|---|---|---|---|
| Template | `kit/my_bot.py` | `kit/rust/` | `kit/cpp/` |
...
Add a new package at the **repository root**, next to `kit/`, not
...
Rename the package in `mybot/Cargo.toml` from `kit` to `mybot`, and
...
Read `kit/sdk.py` and the `MappingBot` class
