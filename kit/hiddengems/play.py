...
def main():
    parser = argparse.ArgumentParser(
        description="Play every round this credential is seated in."
    )
    parser.add_argument(
        "--strategy",
        default="my_bot.py",
    ...
    parser.add_argument(
        "--poll", type=float, default=2.0, help="Seconds between lobby checks"
    )
    parser.add_argument("--once", action="store_true", help="Play one round, then exit")
    parser.add_argument(
        "--example", action="store_true", help="Use the bundled mapping example"
    )
    ...
    token = (
        args.token
        or os.environ.get("HIDDENGEMS_TOKEN")
