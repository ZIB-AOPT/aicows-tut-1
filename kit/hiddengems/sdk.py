...
class MappingBot(BotStrategy):
    ...
    def choose_scan(self, turn):
        return 2 if turn['turn'] % 3 == self.phase and turn['self']['energy'] >= 3 else 0
    ...
    def choose_move(self, o):
        ...
        for gid, (p, expiry) in list(self.gems.items()):
            if expiry <= o['turn'] or p == start or (o['radius'] and abs(p[0]-start[0])+abs(p[1]-start[1]) <= o['radius'] and gid not in visible_ids):
                del self.gems[gid]
        for gem in o['gems']:
            self.gems[gem['id']] = (tuple(gem['position']), o['turn']+gem['ttl'])
        occupied = {tuple(b['position']) for b in o['bots']}
        parent, distance, queue = {start: None}, {start: 0}, deque([start])
        while queue and len(parent) < 2000:
            ...
            for direction, q in self.neighbors(p):
                if self.known.get(q) and q not in occupied and q not in parent:
                    ...
                    queue.append(q)
        goals = [(max(0, expiry-o['turn']-distance[p])/(distance[p]+3), p)
                 for p, expiry in self.gems.values() if p in parent and p != start]
        target = max(goals, default=(0, None))[1]
        if target is None:
            frontiers = [p for p in parent if p != start and any(q not in self.known for _, q in self.neighbors(p))]
            if frontiers:
                target = min(frontiers, key=lambda p: distance[p]+3*self.visits.get(p, 0))
        if target:
            directions = []
            p = target
            while p != start:
                p, direction = parent[p]
            ...
            for direction in directions:
                if length == maximum or direction != directions[0]:
                    break
                length += 1
            return {'direction': directions[0] if length else 'WAIT', 'distance': length}
        options = [(self.visits.get(q, 0)+self.random.random(), direction) for direction, q in self.neighbors(start)
                   if q not in occupied and self.known.get(q, True) and 0 < q[0] < self.config['width']-1 and 0 < q[1] < self.config['height']-1]
...
async def claim_name(session, server, token, name):
    ...
    async with session.post(server.rstrip('/')+'/api/lobby/name', json={'name': name},
...
async def next_round(session, server, token):
    ...
    async with session.get(server.rstrip('/')+'/api/lobby',
                           headers={'Authorization': 'Bearer '+token}) as response:
        if 400 <= response.status < 500:
...
async def run_workshop(server, token, strategy_factory=None, name=None, poll=2.0,
                       once=False, announce=None, session=None):
    ...
    async def loop(client):
        ...
        while True:
            ...
            if match_id is None or match_id in played:
                ...
                continue
            idle = False
            announce(f"Joining {assignment['map']} match {match_id} as "
                     f"{assignment['name']} in slot {assignment['slot']}"
                     +(', waiting for the operator to start it' if assignment['status'] == 'waiting' else ''))
            try:
                result = await run_bot(server, match_id, token, strategy_factory(),
                                       name=assignment['name'], session=client)
...
async def run_bot(server, match_id, token, strategy=None, name='Example mapper', ready=None, session=None):
    strategy = strategy or MappingBot()
    async def play(client):
        async with client.ws_connect(server.rstrip('/')+'/ws/bot/'+match_id,
                                     headers={'Authorization': 'Bearer '+token}, max_msg_size=2**20, heartbeat=20) as ws:
            ...
            async for message in ws:
                ...
                if value['type'] == 'turn':
                    ...
                    await ws.send_json({'type': 'move', 'turn': value['turn'], **action})
