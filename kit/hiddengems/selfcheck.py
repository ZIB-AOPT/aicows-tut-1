...
from .config import HEIGHT, RULESET, WIDTH, GameConfig
...
def own_state(bot_id, position, energy, turn):
    return {
        ...
        "score": turn // 40,
        "captures": turn // 40,
        "feedback": {"distance": 0, "blocked": False, "reason": None},
    }
...
def observation(rng, turn, position, energy, radius):
    ...
    for dx in range(-radius, radius + 1):
        for dy in range(-radius + abs(dx), radius - abs(dx) + 1):
            ...
            terrain.append(
                {"position": list(cell), "kind": "wall" if wall else "floor"}
            )
    ...
    return {
        "type": "observation",
        "turn": turn,
        "self": own_state(0, position, energy, turn),
        "radius": radius,
        "scan_cost": 0,
        "scan_error": None,
        "terrain": terrain,
...
def check(strategy_class, turns, deadline_ms, seed):
    ...
    strategy.initialize(
        {
            ...
            "self": own_state(0, position, energy, 0),
        }
    )
    for turn in range(turns):
        ...
        if type(n) is not int or not 0 <= n <= 10**9:
            problems.append(
                f"turn {turn}: choose_scan returned {n!r}; the server needs an integer from 0"
            )
            n = 0
        ...
        if not isinstance(action, dict) or set(action) != {"direction", "distance"}:
            problems.append(
                f"turn {turn}: choose_move must return exactly direction and distance, got {action!r}"
            )
            continue
        direction, distance = action["direction"], action["distance"]
        if direction not in DIRECTIONS:
            problems.append(
                f"turn {turn}: direction {direction!r} is not one of {', '.join(DIRECTIONS)}"
            )
            continue
        if type(distance) is not int or distance < 0:
            problems.append(
                f"turn {turn}: distance {distance!r} must be an integer from 0"
            )
            continue
        ...
        if distance * distance > energy:
            problems.append(
                f"turn {turn}: {direction} {distance} costs {distance * distance} "
                f"with {energy} energy; the server would make this WAIT"
            )
            continue
        energy -= distance * distance
        step = {"N": (0, -1), "E": (1, 0), "S": (0, 1), "W": (-1, 0), "WAIT": (0, 0)}[
            direction
        ]
        position = (
            min(WIDTH - 2, max(1, position[0] + step[0] * distance)),
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="my_bot.py",
        help="Strategy file, optionally FILE:CLASS",
    )
    ...
    problems, slowest, mean = check(
        strategy_class, args.turns, args.deadline_ms, args.seed
    )
    print(
        f"{args.turns} turns · slowest decision {slowest * 1000:.1f} ms · average {mean * 1000:.1f} ms"
    )
    if slowest * 1000 > args.deadline_ms:
        problems.append(
            f"a decision took {slowest * 1000:.0f} ms, longer than the {args.deadline_ms} ms "
