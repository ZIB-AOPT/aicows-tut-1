...
class MyBot(BotStrategy):
    ...
    def choose_move(self, observation):
        ...
        floor = {tuple(c['position']) for c in observation['terrain'] if c['kind'] == 'floor'}
        ...
        options = [('N', (x, y-1)), ('E', (x+1, y)), ('S', (x, y+1)), ('W', (x-1, y))]
        shift = (observation['turn']//5+observation['self']['id']) % 4
        for direction, cell in options[shift:]+options[:shift]:
            if cell in floor and cell not in blocked and observation['self']['energy'] >= 1:
                return {'direction': direction, 'distance': 1}
        return {'direction': 'WAIT', 'distance': 0}


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Play Hidden Gems with the strategy in this file.')
    ...
    try:
        if args.match:
            result = asyncio.run(run_bot(args.server, args.match, token, MyBot(), args.name or 'My bot'))
            print(json.dumps(result))
        else:
            # No match given: the server tells this token which round to join next.
            asyncio.run(run_workshop(args.server, token, MyBot, name=args.name,
