...
void play(Stream& socket, const Options& options, const Origin& origin, const std::string& token) {
    ...
    // Without a match the server seats this credential in the round it belongs to.
    socket.handshake(origin.authority, "/ws/bot/" + (options.match.empty() ? std::string("current") : options.match));
    socket.text(true);
...
int main(int argc, char** argv) {
    try {
        Options options;
        // The arena URL comes from the environment unless --server overrides it.
        if (const char* configured = std::getenv("HIDDENGEMS_SERVER")) {
            if (*configured) options.server = configured;
        }
        for (int i = 1; i < argc; ++i) {
            const std::string option = argv[i];
            if (option == "--help" || option == "-h") {
                std::cout << "Usage: hiddengems-bot [--server URL] [--name NAME] [--match MATCH_ID]\n"
        ...
        if (!options.match.empty() && !std::regex_match(options.match, std::regex("[0-9a-fA-F]{16}"))) {
            throw std::runtime_error("--match must be the server's 16-character hexadecimal match ID");
        }
        const char* credential = std::getenv("HIDDENGEMS_TOKEN");
        if (!credential || !*credential) credential = std::getenv("HIDDENGEMS_BOT_TOKEN");
        if (!credential || !*credential) throw std::runtime_error("Set HIDDENGEMS_TOKEN to your credential");
